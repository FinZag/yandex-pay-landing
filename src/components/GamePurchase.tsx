...
const GamePurchase = ({ gameId, gameTitle }: { gameId: string; gameTitle: string }) => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!/^[A-Za-z0-9_-]{1,64}$/.test(playerId.trim())) {
      setSoft(false);
      setError('Укажите номер игрока — его видно в игре, в разделе «Настройки».');
      return;
    }
  ...
  return (
    <section id="purchase" className="mx-auto max-w-[1280px] px-5 pt-16 md:px-[76px] md:pt-24">
          ...
          {products === null ? (
            ...
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <h3 className="font-head text-[18px] font-bold">{product.title}</h3>
                  <p className="mt-1 text-[15px] leading-[1.35] text-muted-foreground">
                    Разовая покупка для {gameTitle}. Реклама пропадает навсегда, подписки
                    и автосписаний нет.
                  </p>
                </div>
              ...
              <label className="mt-7 block text-[15px] font-medium" htmlFor="buy-player">
                Номер игрока
              </label>
              <input
                id="buy-player"
              ...
              <p className="mt-2 text-[13px] leading-[1.4] text-muted-foreground">
                Откройте игру → «Настройки» → «Номер игрока». По нему покупка попадёт именно
                в ваш профиль.
              </p>

              <label className="mt-5 block text-[15px] font-medium" htmlFor="buy-email">
                E-mail для чека
              </label>
              <input
                id="buy-email"
              ...
              <label className="mt-4 flex cursor-pointer items-start gap-3 text-[14px] leading-[1.4] text-muted-foreground">
                ...
                <span>
                  Согласен с{' '}
                  <Link to="/legal#offer" className="text-foreground underline underline-offset-2">
                    договором оферты
                  </Link>{' '}
                  и{' '}
                  <Link to="/legal#privacy" className="text-foreground underline underline-offset-2">
              ...
              <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
                <span className="font-head text-[24px] font-bold">Итого: {product.amount} ₽</span>
                <button
                  type="submit"
                  disabled={loading}
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg bg-secondary p-6 md:p-8">
            ...
            <ul className="mt-4 space-y-3 text-[15px] leading-[1.4] text-muted-foreground">
              ...
              <li className="flex gap-2.5">
                <Icon name="Check" size={18} className="mt-0.5 shrink-0 text-ok" />
                Данные карты остаются у банка, сайт их не получает и не хранит.
              </li>
              <li className="flex gap-2.5">
                <Icon name="Check" size={18} className="mt-0.5 shrink-0 text-ok" />
                Сама игра бесплатная — покупка нужна только для отключения рекламы.
              </li>
              <li className="flex gap-2.5">
                <Icon name="Check" size={18} className="mt-0.5 shrink-0 text-ok" />
                <span>
                  Деньги списались, а реклама осталась — напишите на{' '}
                  <MailLink subject={`Покупка в игре ${gameTitle}`} /> с номером заказа, решим
                  вопрос.
                </span>
              </li>
            </ul>
